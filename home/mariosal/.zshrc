...
plugins=(bundler emacs git gitignore nvm rails rbenv)
...
cl() {
  $CC ${=CPPFLAGS} ${=CFLAGS} -pedantic-errors -Og -std=c11 -g -Weverything ${=LDFLAGS} $@ ${=LDLIBS}
}

ccl() {
  $CXX ${=CPPFLAGS} ${=CXXFLAGS} -pedantic-errors -Og -std=c++17 -g -Weverything ${=LDFLAGS} $@ ${=LDLIBS}
}
